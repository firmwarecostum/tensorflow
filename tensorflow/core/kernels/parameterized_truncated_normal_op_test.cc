...
TEST_F(ParameterizedTruncatedNormalOpTest,
  ...
  for (int i = 0; i < 101; ++i) {
    LOG(INFO) << "VAL_RUN2_" << i << ": " << flat_output2(i);
  }
}
...
class StatelessParameterizedTruncatedNormalOpGPUTest : public OpsTestBase {
 protected:
  void Init(DataType dtype) {
    std::unique_ptr<Device> device_gpu(
        DeviceFactory::NewDevice("GPU", {}, "/job:a/replica:0/task:0"));
...
TEST_F(StatelessParameterizedTruncatedNormalOpGPUTest, TestIntegerOverflowGPU) {
  Init(DT_FLOAT);
  if (device_type_ != DEVICE_GPU) return;
